<?php
	...
	function fn_devolverProductoMarca($criterio, $orden){
		...
		$Ssql="SELECT IdProductoMarca, ProductoMarca, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoMarca";
		//echo $Ssql;
		if (!empty($criterio)) {
...
//////////////////////////////////////////////////////////////////////////////////
    function fn_devolverProductoCategoria($criterio, $orden){
		...
		$Ssql="SELECT IdProductoCategoria, ProductoCategoria, IdProductoCategoriaSub, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoCategoria";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
...
FROM
botica.Ve_DocVenta
INNER JOIN botica.Ve_DocVentaPuntoVenta ON Ve_DocVenta.IdDocVentaPuntoVenta = Ve_DocVentaPuntoVenta.IdDocVentaPuntoVenta
INNER JOIN botica.Ve_DocVentaCliente ON Ve_DocVenta.IdCliente = Ve_DocVentaCliente.IdCliente
INNER JOIN botica.Ve_DocVentaTipoDoc ON Ve_DocVenta.IdTipoDoc = Ve_DocVentaTipoDoc.IdTipoDoc
INNER JOIN botica.Lo_Almacen ON Ve_DocVenta.IdAlmacen = Lo_Almacen.IdAlmacen";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
...
FROM
botica.Ve_DocVentaDet
INNER JOIN botica.Gen_Producto ON Ve_DocVentaDet.IdProducto = Gen_Producto.IdProducto ";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
   ...
   function fn_devolverProductoCategoriaSiExiste($ProductoCategoria){
		...
		$Ssql="SELECT IdProductoCategoria, ProductoCategoria, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoCategoria";
		//echo $Ssql;
   ...
   //////////////////////////////////////////////////////////////////////////////////
    function fn_devolverProductoMedicion($criterio, $orden){
		...
		$Ssql="SELECT IdProductoMedicion, ProductoMedicion, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoMedicion";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
   ...
   function fn_devolverProductoMedicionSiExiste($ProductoMedicion){
    ...
    function fn_devolverProductoFormaFarmaceutica($criterio, $orden){
		...
		$Ssql="SELECT IdProductoFormaFarmaceutica, ProductoFormaFarmaceutica, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoFormaFarmaceutica";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
   ...
   function fn_devolverProductoFormaFarmaceuticaSiExiste($ProductoFormaFarmaceutica){
    ...
    function fn_devolverProductoCompuesto($criterio, $orden){
		...
		$Ssql="SELECT IdProductoCompuesto, ProductoCompuesto, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Gen_ProductoCompuesto";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
   ...
   function fn_devolverProductoCompuestoSiExiste($ProductoCompuesto){
    ...
    function fn_devolverProducto($criterio, $orden){
		...
		$Ssql="SELECT
					...
					FROM
					botica.Gen_Producto
					INNER JOIN botica.Gen_ProductoMarca ON Gen_Producto.IdProductoMarca = Gen_ProductoMarca.IdProductoMarca
					INNER JOIN botica.Gen_ProductoFormaFarmaceutica ON Gen_Producto.IdProductoFormaFarmaceutica = Gen_ProductoFormaFarmaceutica.IdProductoFormaFarmaceutica
					INNER JOIN botica.Gen_ProductoMedicion ON Gen_Producto.IdProductoMedicion = Gen_ProductoMedicion.IdProductoMedicion
					INNER JOIN botica.Gen_ProductoCategoria ON Gen_Producto.IdProductoCategoria = Gen_ProductoCategoria.IdProductoCategoria
					LEFT JOIN botica.Gen_ProductoBloque ON Gen_Producto.IdBloque = Gen_ProductoBloque.IdBloque";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
   ...
   /*function fn_devolverProductoCompuestoSiExiste($ProductoCompuesto){
    ...
    function fn_devolverTipoDocVenta($criterio, $orden){
    	$Ssql="SELECT IdTipoDoc, TipoDoc, VaRegVenta, CodSunat FROM botica.Ve_DocVentaTipoDoc";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
    ...
    function fn_devolverAlmacen($criterio, $orden){
    	$Ssql="SELECT IdAlmacen, Almacen, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Lo_Almacen;";
		//echo $Ssql;
		if (!empty($criterio)) {
			$Ssql= $Ssql." WHERE ".$criterio;
		}
    ...
    function fn_devolverPuntodeVenta($criterio, $orden){
    	$Ssql="SELECT IdDocVentaPuntoVenta, PuntoVenta, SerieDocVenta, SerieImpresora, RutaImpresora, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Ve_DocVentaPuntoVenta;";
    	return getSQLResultSet($Ssql);
    }
   ...
   /* function fn_devolverPuntodeVenta($criterio, $orden){
    ...
    function fn_devolverCliente($criterio, $orden){
    	$Ssql = "SELECT IdCliente, Cliente, DniRuc, Direccion, Telefono, Email, Anulado, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Ve_DocVentaCliente;";
    	return getSQLResultSet($Ssql);
    }

		function fn_devolverMetPago()
		{
			$Ssql = "SELECT IdMetodoPago, MetodoPago, EsTarjeta FROM botica.Ve_DocVentaMetodoPago;";
    	return getSQLResultSet($Ssql);
		}
	function fn_devolverSintomas($edad, $criterio){
		$Ssql = "CALL SbVe_ExpertoSintomaBuscar ($edad, '$criterio');";
		return getSQLResultSet($Ssql);
	}

	function fn_devolverDiagnostico(){
		$Ssql = "SELECT IdDiagnostico, Diagnostico, Problema, Edad, FechaReg, UsuarioReg, FechaMod, UsuarioMod FROM botica.Ve_ExpertoDiagnostico;";
		return getSQLResultSet($Ssql);
	}

	function fn_devolverDiagnosticoSintoma($edad, $criterio){
		$Ssql="CALL SbVe_ExpertoDiagnosticoXSintomaBuscar ($edad, $criterio);";
		return getSQLResultSet($Ssql);
	}
	...
	{
		$Ssql = "CALL botica.SbLo_Stock('$almacen', '$producto');";
		return getSQLResultSet($Ssql);
	}
	...
	function ListarReporteKardex($producto, $fechaIni, $fechaFin)
	{
		$Ssql = "call SbLo_Kardex('$producto', '$fechaIni', '$fechaFin');";
		return getSQLResultSet($Ssql);
	}
