...
function fn_modificarProductoCompuesto($idProductoCompuesto,$productoCompuesto,$anulado,$usuario){
    ...
    function fn_eliminarProductoCompuesto($idProductoCompuesto){
		...
		function fn_guardarProducto($idProductoMarca, $idPoductoFormaFarmaceutica, $idProductoMedicion, $idProductoCategoria, $bloque,  $producto, $productoDesc, $productoDescCorto, $codigoBarra, $codigo, $dosis,  $precioCosto,$ventaEstrategica, $precioUtilidad, $precioContado,$precioXMayor,$stockXMayor, $controlaStock, $stockMin,$usuario){
		    ...
		    function fn_guardarDocVenta($puntoVenta, $tipoDoc, $cliente, $almacen, $usuario, $EsCredito, $fechaCredito, $serie){

				$fechaDoc=fn_devolverfechaActual();

				if(!$fechaCredito) {
					$Ssql="CALL SbVe_GuardarDocVenta ('$puntoVenta', '$tipoDoc', '$cliente', '$almacen', '$fechaDoc', '$usuario', $EsCredito, NULL, '$serie')";
				} else {
					$Ssql="CALL SbVe_GuardarDocVenta ('$puntoVenta', '$tipoDoc', '$cliente', '$almacen', '$fechaDoc', '$usuario', $EsCredito, '$fechaCredito', '$serie')";
				}
				//var_dump($Ssql);exit();
			...
			function fn_guardarMovimientoDet($hash, $producto, $cantidad, $precio, $tieneIgv, $ISC, $Flete, $Lote, $fechaVen, $pesoTotal, $precioTipoCambio, $precioContado){
		    	$Ssql = "CALL SbLo_MovimientoDetGuardar('$hash', '$producto', $cantidad, $tieneIgv, $precio,$ISC, $Flete, $Lote, '$fechaVen', '$pesoTotal', '$precioContado')";
					//echo $Ssql;
					//exit();
					ejecutarSQLCommand($Ssql);
		    }
