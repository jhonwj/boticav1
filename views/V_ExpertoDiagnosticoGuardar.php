...
if (isset($dataSintoma)) {
	...
	if (sizeof($dataTratamiento)>0) {
		$Sql = "UPDATE Ve_ExpertoTratamiento SET Ve_ExpertoTratamiento.IdDiagnostico = 0 WHERE Ve_ExpertoTratamiento.IdDiagnostico = ".$dataDiagnostico[4];
	...
	foreach ($dataTratamiento as $key) {
		...
		echo "Entrooo";
		$Sql = "DELETE FROM Ve_ExpertoTratamiento WHERE Ve_ExpertoTratamiento.IdDiagnostico = ".$dataDiagnostico[4];
		$res = ejecutarSQLCommand($Sql);
	}
	if (isset($dataSintoma)) {
	$Sql = "DELETE FROM Ve_ExpertoDiagnosticoSintomaDet WHERE Ve_ExpertoDiagnosticoSintomaDet.IdDiagnostico = ".$dataDiagnostico[4];
	$res = ejecutarSQLCommand($Sql);
	foreach ($dataSintoma as $key) {
